  ...
  return (
    <form action={formAction} className="mt-3">
      ...
      <button
        type="submit"
        disabled={isPending}
        className="rounded-full bg-[#556B2F] px-4 py-2 text-xs font-semibold text-white"
      >
...
export function ConfirmedGiftsPanel({ gifts }: ConfirmedGiftsPanelProps) {
  if (gifts.length === 0) {
    return (
      <p className="rounded-2xl bg-white p-4 sm:rounded-3xl sm:p-8 text-sm text-stone-600 shadow-sm">
        Todavía no recibiste regalos confirmados.
      </p>
    );
  }
          ...
          {gift.voucherUrl ? (
            <p className="mt-3 text-sm">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[#556B2F] underline"
              >
                Ver comprobante
              </a>
            </p>
